...
public class Client {
		...
		try {
			while (!endOfQueue) {
				...
				if (recieved instanceof Message) {
					...
					this.sendReadConfirmation ((Message) recieved);
	...
	private void sendReadConfirmation (Message message) throws Exception {
		ReadConfirmation confirmation = new ReadConfirmation (message);
		this.output.writeObject (confirmation);
	}
